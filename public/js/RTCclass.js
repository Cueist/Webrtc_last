'use strict';
// with new version of JS. Maybe, you don't want to use 'this'.
class Webrtc extends EventTarget {
	...
	_connect(socketId) {
		if (typeof this._localStream !== 'undefined' && this.isReady) {
			...
			// .addStream() is deprecated. .addTrack() ile değiştirilmelidir.
			// Ancak .addTrack() tamamen farklı çalışıyor.

			// What they said (not working yet!)
			// this.localStream
			// 	.getTracks()
			// 	.forEach((track) =>
			// 		this.pcs[socket.id].addTrack(track, this.localStream),
			// 	);
			// this.localStream
			// 	.getTracks()
			// 	.forEach((track) => pcs[socketId].addTrack(track, stream));

			// legacy addStream() the working one
			...
			//   this.pcs[this.socketId].addTrack(track);
			// }
	...
	_onSocketListeners() {
		...
		// Share Screen
		// this.socket.on('shareScreen', (socketId) => {
		// 	this._shareScreen(socketId);
		// });
	...
	_handleOnRemoveTrack(socketId, event) {
		this.log('Remote stream added for ', socketId);
		...
		const senders = this.pcs[socketId].getSenders();

		const sender = senders.find((sender) => {
			return (
				sender.track.kind === event.streams[socketId].getVideoTracks()[0].kind
			);
		});

		if (sender) {
			sender.replaceTrack(event.streams[socketId].getVideoTracks()[0]);
		}
	}
	...
	_shareScreen = async (socketId) => {
		...
		try {
			...
			// localVideo.setAttribute('hidden', 'true');
			// localScreen.setAttribute('hidden', 'false');
			// localScreen.srcObject = screenSharingStream;
			// localScreen.addEventListener('loadedmetadata', () => {
			// 	localScreen.play();
			// });
		} catch (err) {
			console.error(
