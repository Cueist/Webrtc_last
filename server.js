...
const options = {
	pfx: fs.readFileSync('keystore.pfx'),
	passphrase: 'changeit',
};
...
io.sockets.on('connection', function (socket) {
	...
	// // share-Screen useless (webrtc already does that)
	// socket.on("share-screen", (screen, room) => {

	//   socket.broadcast.to(room).emit("share-screen", screen, socket.id);
	...
	// socket.on('share-screen', (socketId) => {
	// 	socket.emit('shareScreen', socketId);
	// 	// io.sockets.sockets.get(socketId).shareScreen(socketId);
	// });
	 ...
	 * (bu kısım javadoc gibi kullanılıyor)
	 * bunun yazıldığı yere yapışık olması gerekiyor
	 */
	socket.on('disconnecting', () => {
		// bu yüzden comment'ler buraya bu adam hangi room'dan leave yapıyor. ekle
